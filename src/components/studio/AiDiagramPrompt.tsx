...
function generate(prompt: string): ElementShape[] {
  ...
  if (["mind map", "brainstorm", "ideas"].some((k) => p.includes(k))) {
    ...
    branches.forEach((b, i) => {
      ...
      const x = 520 + Math.cos(angle) * 300;
      const y = 320 + Math.sin(angle) * 180;
  ...
  if (["timeline", "roadmap", "schedule"].some((k) => p.includes(k))) {
    ...
    items.forEach((item, i) => {
      const x = 130 + i * 220;
      out.push(base("line", x, 280, 0, 40), textEl(item, x - 20, 235));
    });
    return out;
  }
  ...
  if (["pros cons", "comparison", " vs ", "vs"].some((k) => p.includes(k))) {
    ...
    const pros = items.slice(0, mid);
    const cons = items.slice(mid);
